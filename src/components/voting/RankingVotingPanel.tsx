          ...
          <div className="border-t border-[#dfe5f4] p-4">
            ...
            {votingState === 'closed' && (
              <div className="mt-4 border-t border-[#dfe5f4] pt-4">
                ...
                <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="text-[10px] text-[#8a94a3]">5~1000자 · 확정/폐기 기록은 삭제할 수 없습니다.</span><div className="flex flex-wrap gap-2"><button type="button" disabled={isPending || totalVotes < 1 || !hasValidReason} onClick={finalizeVote} className="inline-flex items-center gap-1.5 rounded-xl bg-[#3457c8] px-3 py-2 text-xs font-black text-white hover:bg-[#2445ad] disabled:bg-[#e5e8ed] disabled:text-[#9aa3af]"><Trophy className="h-3.5 w-3.5" />결과 확정</button><button type="button" disabled={isPending || !hasValidReason} onClick={voidVoteRound} className="inline-flex items-center gap-1.5 rounded-xl border border-[#efc2ca] bg-white px-3 py-2 text-xs font-black text-[#be4057] hover:bg-[#fff1f2] disabled:border-[#e5e8ed] disabled:text-[#a0a8b3]"><Trash2 className="h-3.5 w-3.5" />라운드 폐기</button></div></div>
              </div>
            )}
          </div>
        </details>
