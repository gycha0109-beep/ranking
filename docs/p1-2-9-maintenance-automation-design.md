...
- 기존 service-role RPC signature와 result type을 유지한다.
...
- 해결된 신고 자유서술 상세 비식별화
- Cron 자체 실행 이력 보존기간 정리
- 관리자 유지보수 상태 화면
- 기존 통합 운영 감사에 유지보수 실행 요약 추가
- Hosted에서 수동 실행, 실패 기록, 권한, schedule 상태 검증
...
Cron 세션은 API의 `service_role` JWT가 아니므로 기존 RPC를 그대로 호출하면 `auth.role()` 검사에서 거부된다. 실제 데이터 변경 로직을 private batch helper로 이동하고, 기존 public RPC와 Cron runner가 같은 helper를 호출한다.
...
| `prune_cron_history` | 매일 03:50 | 5,000 | 10 | 30일 이전 `cron.job_run_details` 삭제 |

신고 원본 행과 신고 결정 원장은 보존한다. `comment_reports.details`만 `NULL`로 바꾸고 `details_redacted_at`을 기록한다. 사용자 유지보수 실행 원장은 삭제하지 않는다.
...
1. Cron 호출은 `session_user = 'postgres'`, API 호출은 `auth.role() = 'service_role'`인지 확인
2. 작업 정의 조회
3. 작업별 transaction advisory lock을 `try` 방식으로 획득
4. `statement_timeout` 설정
5. nested exception block 안에서 batch helper를 최대 `max_batches`만큼 반복
6. batch 결과가 batch 크기보다 작거나 0이면 종료
7. 성공 또는 no-work 실행 결과를 outer block에서 원장에 INSERT
8. 예외 발생 시 nested subtransaction의 작업 변경을 rollback하고 outer block에서 실패 원장을 INSERT
9. 오류를 재전파하지 않고 JSON 결과 반환
...
SECURITY DEFINER의 `current_user`는 호출자 식별에 사용하지 않는다.

## 8. 실패 및 재시도

실패 시 runner는 해당 실행을 `failed`로 기록하고 예외를 외부로 재전파하지 않는다. 다음 schedule이 같은 작업을 다시 실행하므로 bounded retry가 된다.

runner의 실제 데이터 변경 loop는 nested PL/pgSQL block에서 실행한다. 오류가 발생하면 해당 subtransaction의 모든 batch 변경은 rollback되며 outer block은 `affected_rows = 0`인 실패 원장을 기록한다.

오류 메시지는 1,000자로 제한하며 데이터 원문을 포함하지 않는다. Cron infrastructure 자체 오류는 `cron.job_run_details`에서 별도로 확인한다.
...
- `ranking-maint-prune-cron-history`

동일 migration을 재적용하거나 schedule을 변경해도 중복 Job이 남지 않아야 한다. definition의 `enabled`가 false라면 Cron 호출이 남아 있어도 runner가 작업을 실행하지 않고 `disabled`를 기록한다.
...
- advisory try-lock 구현 및 `skipped_locked` 분기 정의 확인
- 잘못된 job key 거부 계약 확인
- 원장 UPDATE/DELETE 차단 확인
- 관리자 상태 RPC와 통합 감사 결과 확인
- Cron job run history 보존기간 helper 확인
