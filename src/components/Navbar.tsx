...
import { Bell, Bookmark, Shield, LogIn, LogOut, LayoutDashboard, Compass } from 'lucide-react'
          ...
          {user ? (
            <>
              <Link
                ...
                className="relative flex h-9 w-9 items-center justify-center rounded-xl border border-white/[0.07] bg-white/[0.025] text-slate-400 transition hover:border-indigo-500/25 hover:bg-indigo-500/10 hover:text-indigo-200"
                title="내 알림"
                aria-label={`내 알림${unreadNotificationCount > 0 ? `, 읽지 않음 ${unreadNotificationCount}개` : ''}`}
              >
                <Bell className="h-4 w-4" />
                {unreadNotificationCount > 0 && (
                  <span className="absolute -right-1.5 -top-1.5 min-w-5 rounded-full border-2 border-[#07070a] bg-indigo-400 px-1 text-center text-[8px] font-black leading-4 text-indigo-950">
              ...
              <form action={async () => {
                ...
                <button type="submit" className="p-2 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-transparent hover:border-rose-500/10 transition-all flex items-center justify-center" title="로그아웃">
                  <LogOut className="w-4 h-4" />
                </button>
              </form>
            </>
          ) : (
            <Link href="/login" className="px-4 py-2 rounded-xl text-xs font-bold text-slate-200 bg-white/[0.04] hover:bg-white/[0.08] border border-white/10 hover:border-white/20 transition-all flex items-center gap-1.5">
