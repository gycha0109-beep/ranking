...
CREATE TABLE IF NOT EXISTS public.ranking_semantic_projections (
  ...
  CONSTRAINT ranking_semantic_projections_subject_key_check CHECK (subject_key ~ '^[a-z0-9][a-z0-9._/-]{0,127}$'),
  CONSTRAINT ranking_semantic_projections_intent_key_check CHECK (intent_key IS NULL OR intent_key ~ '^[a-z0-9][a-z0-9._/-]{0,127}$'),
  CONSTRAINT ranking_semantic_projections_method_key_check CHECK (method_key IS NULL OR method_key ~ '^[a-z0-9][a-z0-9._/-]{0,127}$'),
  CONSTRAINT ranking_semantic_projections_coordinates_object_check CHECK (jsonb_typeof(coordinates) = 'object'),
  CONSTRAINT ranking_semantic_projections_version_coordinates_object_check CHECK (jsonb_typeof(version_coordinates) = 'object'),
  CONSTRAINT ranking_semantic_projections_classification_state_check CHECK (classification_state IN ('inferred', 'reviewed')),
  CONSTRAINT ranking_semantic_projections_confidence_check CHECK (confidence >= 0 AND confidence <= 1),
  CONSTRAINT ranking_semantic_projections_projection_version_check CHECK (length(btrim(projection_version)) BETWEEN 1 AND 120)
);
...
BEGIN
  ...
  NEW.claim_signature := 'ia2:claim:' || md5(concat_ws('|', NEW.subject_key, coalesce(NEW.intent_key, ''), NEW.coordinates::text));
  NEW.view_signature := 'ia2:view:' || md5(concat_ws('|', NEW.subject_key, coalesce(NEW.intent_key, ''), NEW.coordinates::text, coalesce(NEW.method_key, '')));
  NEW.version_signature := 'ia2:version:' || md5(concat_ws('|', NEW.subject_key, coalesce(NEW.intent_key, ''), NEW.coordinates::text, coalesce(NEW.method_key, ''), NEW.version_coordinates::text));
  NEW.updated_at := now();
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_ia_2_refresh_projection_signatures ON public.ranking_semantic_projections;
CREATE TRIGGER trg_ia_2_refresh_projection_signatures
BEFORE INSERT OR UPDATE ON public.ranking_semantic_projections
FOR EACH ROW EXECUTE FUNCTION private.ia_2_refresh_projection_signatures();

CREATE INDEX IF NOT EXISTS idx_ranking_semantic_projections_subject_key ON public.ranking_semantic_projections(subject_key, ranking_id);
CREATE INDEX IF NOT EXISTS idx_ranking_semantic_projections_claim_signature ON public.ranking_semantic_projections(claim_signature, ranking_id);
CREATE INDEX IF NOT EXISTS idx_ranking_semantic_projections_view_signature ON public.ranking_semantic_projections(view_signature, ranking_id);
CREATE INDEX IF NOT EXISTS idx_ranking_semantic_projections_version_signature ON public.ranking_semantic_projections(version_signature, ranking_id);

ALTER TABLE public.ranking_semantic_projections ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS ranking_semantic_projections_public_read ON public.ranking_semantic_projections;
...
WITH seed(slug, subject_key, intent_key, coordinates, method_key, version_coordinates, confidence, projection_version) AS (
  VALUES
...
INSERT INTO public.ranking_semantic_projections (
  ranking_id, subject_key, intent_key, coordinates, method_key, version_coordinates,
  classification_state, confidence, projection_version, claim_signature, view_signature, version_signature
)
SELECT r.id, seed.subject_key, seed.intent_key, seed.coordinates, seed.method_key, seed.version_coordinates,
       'inferred', seed.confidence, seed.projection_version, '', '', ''
FROM seed JOIN public.rankings r ON r.slug = seed.slug
