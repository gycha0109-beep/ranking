...
CREATE INDEX idx_admin_role_change_events_created ON public.admin_role_change_events(created_at DESC, id DESC);
CREATE INDEX idx_admin_role_change_events_target ON public.admin_role_change_events(target_user_id, created_at DESC, id DESC);

INSERT INTO public.admin_role_change_events(target_user_id, previous_level, new_level, actor_id, reason)
SELECT DISTINCT ur.user_id, 'admin'::TEXT, 'super_admin'::TEXT, NULL::UUID,
  'P1-2.8 역할 분리 도입에 따라 기존 관리자 권한을 최고 관리자 수준으로 이관했습니다.'::TEXT
FROM public.user_roles ur WHERE ur.role = 'admin';

INSERT INTO public.user_roles(user_id, role)
SELECT DISTINCT user_id, 'moderator' FROM public.user_roles WHERE role = 'admin'
ON CONFLICT (user_id, role) DO NOTHING;
INSERT INTO public.user_roles(user_id, role)
SELECT DISTINCT user_id, 'super_admin' FROM public.user_roles WHERE role = 'admin'
ON CONFLICT (user_id, role) DO NOTHING;
...
FOR EACH ROW EXECUTE FUNCTION private.reject_admin_role_event_mutation();

CREATE OR REPLACE FUNCTION private.get_admin_role_level(p_user_id UUID)
RETURNS TEXT LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public, pg_temp AS $$
  SELECT CASE
    WHEN EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=p_user_id AND role='super_admin') THEN 'super_admin'
    WHEN EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=p_user_id AND role='admin') THEN 'admin'
    WHEN EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=p_user_id AND role='moderator') THEN 'moderator'
    ELSE 'none'
  END;
$$;
...
BEGIN
  ...
  IF v_capability IN ('admin_console_access','moderation_review','report_review','sanction_view','sanction_impose_warning') THEN
    RETURN v_level IN ('moderator','admin','super_admin');
  END IF;
  IF v_capability IN ('content_manage','sanction_impose_restriction','appeal_reject','audit_view') THEN
    RETURN v_level IN ('admin','super_admin');
  END IF;
  IF v_capability IN ('sanction_impose_long_suspension','sanction_revoke','appeal_accept','role_manage') THEN
...
RETURNS VOID LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = auth, private, pg_temp AS $$
BEGIN
...
RETURNS JSONB LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path = auth, private, pg_temp AS $$
DECLARE v_user_id UUID:=auth.uid(); v_level TEXT; v_capabilities JSONB;
BEGIN
  IF v_user_id IS NULL THEN RETURN jsonb_build_object('role_level','none','capabilities','[]'::JSONB); END IF;
  v_level:=private.get_admin_role_level(v_user_id);
  SELECT COALESCE(jsonb_agg(capability ORDER BY ord),'[]'::JSONB) INTO v_capabilities
  FROM (VALUES
    (1,'admin_console_access'),(2,'moderation_review'),(3,'report_review'),(4,'sanction_view'),
    (5,'sanction_impose_warning'),(6,'content_manage'),(7,'sanction_impose_restriction'),
    (8,'appeal_reject'),(9,'audit_view'),(10,'sanction_impose_long_suspension'),
    (11,'sanction_revoke'),(12,'appeal_accept'),(13,'role_manage')
...
REVOKE ALL ON FUNCTION private.reject_admin_role_event_mutation() FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.get_admin_role_level(UUID) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.has_admin_capability(UUID,TEXT) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.assert_admin_capability(TEXT) FROM PUBLIC, anon, authenticated;
