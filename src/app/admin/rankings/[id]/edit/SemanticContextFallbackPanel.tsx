...
export default function SemanticContextFallbackPanel({
  workspace,
  contextSuggestions,
}: {
  workspace: RankingSemanticWorkspace
  contextSuggestions: RankingSubjectContextSuggestion[]
}) {
  const [subjectQuery, setSubjectQuery] = useState(workspace.projection?.subject_key || '')
  ...
  const lexicalSuggestions = useMemo(
    () => rankRankingSubjectSuggestions(subjectQuery, workspace.subject_options)
      .filter(suggestion => suggestion.subject_key !== normalizedSubjectKey),
    [subjectQuery, normalizedSubjectKey, workspace.subject_options]
  )

  const exactAlias = workspace.subject_aliases.some(
  ...
  if (!contextSuggestion) return null
