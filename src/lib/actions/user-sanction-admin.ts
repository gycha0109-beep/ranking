...
export async function loadUserSanctionAdminData() {
  try {
    const supabase = await requireAdminCapability('sanction_view')
    ...
    return {
      sanctions: Array.isArray(sanctions.data) ? sanctions.data : [],
      appeals: Array.isArray(appeals.data) ? appeals.data : [],
    }
  } catch (error) {
    return {
      sanctions: [],
      appeals: [],
      error: error instanceof Error ? error.message : '제재 운영 데이터를 불러오지 못했습니다.',
    }
  }
}
...
export async function revokeUserSanction(formData: FormData) {
  const supabase = await requireAdminCapability('sanction_revoke')
...
export async function reviewUserSanctionAppeal(formData: FormData) {
  ...
  const supabase = await requireAdminCapability(decision === 'accepted' ? 'appeal_accept' : 'appeal_reject')
