...
export default async function UserSanctionsAdminPage() {
  const [data, access] = await Promise.all([loadUserSanctionAdminData(), getMyAdminAccess()])
  const capabilities = new Set(access.capabilities)
  const canWarn = capabilities.has('sanction_impose_warning')
  const canRestrict = capabilities.has('sanction_impose_restriction')
  const canLongSuspend = capabilities.has('sanction_impose_long_suspension')
  const canRejectAppeal = capabilities.has('appeal_reject')
  const canAcceptAppeal = capabilities.has('appeal_accept')
  const canRevoke = capabilities.has('sanction_revoke')
  const canImpose = canWarn || canRestrict || canLongSuspend
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0a0a0f] to-[#07070a] px-4 py-10 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl space-y-8">
        <header className="border-b border-white/[0.06] pb-6">
          ...
          <p className="mt-2 text-sm text-slate-500">역할별 capability에 따라 경고, 기간 제한, 장기 정지, 해제 및 이의제기 결정을 분리합니다.</p>
        </header>
          ...
          <section className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5 sm:p-6">
            ...
            <form action={imposeUserSanction} className="mt-5 grid gap-3 md:grid-cols-2">
              ...
              <select name="sanctionType" required className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm">
                {canWarn && <option value="warning">경고</option>}
                {canRestrict && <option value="comment_restriction">댓글 제한</option>}
                {canRestrict && <option value="report_restriction">신고 제한</option>}
                {(canRestrict || canLongSuspend) && <option value="account_suspension">계정 활동 제한</option>}
              </select>
              ...
              <input name="durationHours" type="number" min="1" max={canLongSuspend ? 8760 : 168} placeholder={canLongSuspend ? '기간(시간), 경고는 비워둠' : '기간(시간), 최대 168시간'} className="rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm" />
            ...
            {!canLongSuspend && canRestrict && <p className="mt-3 text-xs text-slate-500">7일을 초과하는 계정 활동 제한은 최고 관리자만 결정할 수 있습니다.</p>}
          </section>
        )}
          ...
          {data.appeals.map((raw) => {
            ...
            return (
              <article key={String(row.appeal_id)} className="rounded-2xl border border-indigo-500/15 bg-indigo-500/[0.05] p-5">
                ...
                {(canAcceptAppeal || canRejectAppeal) && (
                  <form action={reviewUserSanctionAppeal} className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]">
                    ...
                    {canAcceptAppeal && <button name="decision" value="accepted" className="rounded-xl border border-emerald-500/25 bg-emerald-500/15 px-4 py-2 text-xs font-black text-emerald-200">수용</button>}
                    {canRejectAppeal && <button name="decision" value="rejected" className="rounded-xl border border-rose-500/25 bg-rose-500/15 px-4 py-2 text-xs font-black text-rose-200">기각</button>}
                  </form>
                )}
              </article>
            )
            ...
            return (
              <article key={String(row.sanction_id)} className="rounded-2xl border border-white/[0.07] bg-white/[0.025] p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  ...
                  {canRevoke && active && row.appeal_status !== 'pending' && (
