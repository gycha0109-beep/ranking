...
export default async function CategoryPage({ params, searchParams }: Props) {
  ...
  const nextHref = page.nextCursor
    ? `/categories/${categorySlug}?${new URLSearchParams({ sort, cursor: page.nextCursor }).toString()}`
    : null
        ...
        <div className="space-y-6 pt-6 border-t border-white/[0.04]">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-white/[0.06] bg-white/[0.02] p-1">
              ...
              <Link
                href={`/categories/${categorySlug}?sort=popular`}
          ...
          {rankings.length > 0 ? (
            ...
            <div className="glass-card rounded-2xl p-12 text-center flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-slate-500 text-xs max-w-sm leading-relaxed">
                현재 이 카테고리에 등록되거나 공개 발행된 랭킹 문서가 존재하지 않습니다.
              </p>
            </div>
          )}
        </div>
