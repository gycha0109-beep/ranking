...
const bridgePath = path.join(root, 'src/app/admin/rankings/[id]/edit/SemanticContextFallbackPanel.tsx')
...
expect(pageSource.includes('<SemanticProjectionPanel initialWorkspace={semanticWorkspace} />'), 'IA-2B semantic panel mount must remain direct')
expect(pageSource.includes('getRankingSubjectContextSuggestions(id)'), 'edit page does not load context suggestions')
expect(pageSource.includes('SemanticContextFallbackPanel'), 'edit page does not mount context fallback sibling')
